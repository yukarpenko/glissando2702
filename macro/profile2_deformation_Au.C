/** \file profile2_deformation_Au.C
 * Script generating x-y and r-cos(theta) distributions of the nucleons in the nuleus 
 * (part of GLISSANDO 2)
 * 
 */
...
r_costheta_hist->SetTitle("r-cos(#theta) profile, nucleus Au (deformed, #beta2 = -0.13, #beta4 = -0.03)");
...
r_costheta_hist_sym->SetTitle("r-cos(#theta) profile, nucleus Au (spherical)");
...
c1->Modified();
c1->Update();
c1->SaveAs("rcostheta_spherical_deformed_Au.eps");
